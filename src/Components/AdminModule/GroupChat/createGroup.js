...
import { getEmployees, createGroup, getAdminGroupDetails } from './network';
import { reducer, initialState } from './reducer';
import RootLoader from '../../Common/Loader/RootLoader';
import * as actions from './action';
import { CREATE_GROUP } from '../../Common/Headers';
...
export default function AddGroupChat(props) {
  ...
  const handleChange = (e) => {
    ...
    state.employees.filter(empp =>empp.workingStatus === "1").map((emp) => {
        return (
            (extension === 'agile24x7.com')?((emp.email !== getUser.user.userName) ?
            empList.push({
                'value': emp.email, 'label': emp.name
            }): ""):((emp.email !== getUser.user.userName &&emp.email.includes(extension)) ?
            empList.push({
                'value': emp.email, 'label': emp.name
            }): "")
        );
                        ...
                        <div className="user-modal-dialog d-flex justify-content-center">
                            <div className="modal-content col-lg-6 p-2" style={{ borderRadius: '10px' }}>
                                <div className="modal-header" style={{ backgroundColor: '#D2ECDF' }}>
                                    <h5 className="modal-title p-2">{CREATE_GROUP}</h5>
                                ...
                                <div className="modal-footer">
                                    {state.isLoading ? <RootLoader /> :
                                     ...
                                     onClick={(event) => { createGroup(selectedValue,state, mem.toString(), label.toString(), dispatch, getUser.user,props.handleClose) }}
                                     >Save</button>
                                    } 
