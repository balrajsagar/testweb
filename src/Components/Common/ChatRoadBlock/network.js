...
import { groupEmail, groupList, groupName, taskComments } from "./actions";
...
export async function getMessages(dispatch,getUser,data) {
  ...
  export async function groupMembersEmail(dispatch,getUser,data) {
    dispatch(isLoading());
    try {
      ...
      //Getting the Group details for roadblock start
        ...
        function getUnique(array) {
         ...
         h = uniqueNames;
        console.log("in",uniqueNames+h.length)
        details = "Admin(admin)";
        for (i = 1; i <= h.length ; i++) {//Checking the group list
