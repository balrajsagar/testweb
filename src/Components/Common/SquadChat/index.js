...
// for sending file
    const sendMessage = async () => {
        // console.log(props.data.player_id)
        if (state.comment !== "") {
         setDisable(true)
            dispatch(isLoading());
            try {
                ...
                var response = await API.post("squad_chat.php", {
                    ...
                    device_id:props.data.device_id, //for notifications
                    player_id:props.data.player_id, //for notifications
...
// for sending file or images
    ...
    // for getting pasted images details
    const myPaste = (e) =>{
    ...
    // console.log("geituser",getUser.user)
        ...
        <div>
            ...
            return (
                ...
                <p className="text-left pr-3"  >
                    <p className="pt-2" style={{color:'green'}}>{comments.username}&nbsp;
                    <q style={{color:'grey',fontSize:"8px"}}>{convertPSTtoLocalTime(comments.messagedTime)}</q>
                </p>
            { comments.path ?
...
//user messages     user message    user  messages
           <p className="text-right pr-3" >
            <p className="pt-2" style={{color:'green'}}>{comments.username}&nbsp;
           {/* <q style={{color:'grey',fontSize:"8px"}}>{moment(comments.messagedTime).format("MM-DD-YYYY h:mm:ss a")}</q> */}
            <q style={{color:'grey',fontSize:"8px"}}>{convertPSTtoLocalTime(comments.messagedTime)}</q>
           </p>
            {comments.path ?
