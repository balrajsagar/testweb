...
import { getEmployees, createGroup, getGroupDetails } from './network';
import { reducer, initialState } from './reducer';
import RootLoader from '../Loader/RootLoader';
import * as actions from './action';
import { CREATE_GROUP } from '../Headers';
                                    ...
                                    <div className="modal-footer">
                                        {state.isLoading ? <RootLoader /> :
                                            <button type="button" className="btn btn-outline-success" style={{ borderRadius: '20px' }}
                                                onClick={(event) => { createGroup(state, mem.toString(), label.toString(), dispatch, getUser.user, props.handleClose) }}
                                            >Save</button>
                                        }
