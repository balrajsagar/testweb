...
import {
  ...
  updateUserStoryToKanban,
  // addCommentUserStory,
  getCurrentSprintUserStoriesToKanban,
  getWorkingHours,
  getWorkingDays,
} from "./network";
...
import { ROADBLOCK, NEWMODULE, COMMENTS, ALL_USERS, CHAT, UPDATE_STORY, USER_STORY_PROGRESS, LIMITED_ACCESS_CONTRIBUTOR, KANBAN, NEWMAINTASK } from "../../Common/Headers";
...
import { MAINTASK_DESCRIPTION, STORY_POINTS, MAINTASK, ACCEPTANCE_CRITERIA, DEFINITION_OF_DONE, PROJECTNAME, ASSIGNED_DATE, PROJECT_COMPLETED_DATE } from '../../Common/Headers';
...
import AddKanban from "../../Common/Modules/addKanban";
...
export default function UserKanban() {
  ...
  useEffect(() => {
    getActiveKanbans(dispatch, getUser.user);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      getActiveKanbans(dispatch, getUser.user, false);// for active sprint user stories
  ...
  function handleCardMove(from, to, data, sprintId) {
    ...
    if (check === "ToDoDoing") {
      ...
      updateUserStoryToKanban(dispatch, getUser.user, data, sprintId, handleClose, message); //updated with sprintId for user stories updated in sprint cycle -->GNK --> Version1.0.6.03
          ...
          {state.modules.length > 0 ? (
            <div style={{ flexDirection: "row" }}>
              <div className="col-12 row p-2">
                ...
                {/* <label className="p-2">{KANBAN}</label>
                <Select
                  ...
                  options={sprints}
                /> */}
                <div style={{ marginLeft: "300px" }} className="mb-2">
                  {(state.modules.length === 0) ? <button
                  ...
                  >
                    ...
                    />
                    <span className="m-1">Add {KANBAN}</span>
                  </button> :
                    <button
                      style={{ backgroundColor: "transparent", border: "0" }}
                      type="button"
                      onClick={() => handleOpen("addUserStory", "")}
                    >
                      {" "}
                      <img
                        src="images/common/add.png"
                        ...
                        style={{ width: "20px", height: "20px" }}
                      />
                      <span className="m-1">{NEWMAINTASK}</span>
                    </button>
                  }
                  {openModule.action === "addSprint" ? (
                    <AddKanban
                      ...
                      handleModalClose={handleModalClose}
                    />
                  ) :openModule.action === "addUserStory" && <AddMainTaskToKanban
                    open={openModule.status}
                    handleClose={handleClose}
                    data={info.activeSprint}
                    handleModalClose={handleModalClose}
                  />}
                </div>
              </div>
